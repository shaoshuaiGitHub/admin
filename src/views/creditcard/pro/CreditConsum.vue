      ...
      <div class="ant-pro-grid-content">
        <div class="ant-page-header">
          ...
          <div class="ant-page-header-heading">
            <span class="ant-page-header-heading-title">消费计划</span>
          </div>
        </div>
      </div>
    </div>
    <div class="wrap-children-content">
      ...
      >
        ...
        <a-table
          ...
          :bordered="true"
        >
          <template slot="consumDate" slot-scope="text">{{ text.substring(0,11)}}</template>
          <template slot="stype" slot-scope="text,record">{{ dateCompare(record.consumDate) }}</template>
          <template
            slot="minMoney"
            slot-scope="text,record"
          >{{ record.minMoney+' - '+record.maxMoney}}</template>
          <template slot="action" slot-scope="text, record">
            <a-button
              class="status-detail"
              style="margin-right:10px;"
              :disabled="record.signMoney >= record.maxMoney || record.statu == 1"
        ...
        >
          <a-form :form="modifyform" @submit="modifySubmit">
            <a-form-item label="请输入已消费金额" :label-col="{ span: 8 }" :wrapper-col="{ span: 12 }">
...
const columns = [
  ...
  {
    title: "计划日期",
    dataIndex: "consumDate",
    scopedSlots: { customRender: "consumDate" }
  },
  {
    title: "计划消费范围",
    dataIndex: "minMoney",
  ...
  methods: {
    ...
    modifySubmit(e) {
      ...
      that.modifyform.validateFields((err, values) => {
        ...
        console.log( that.editValue.maxMoney)
        Number(values.signMoney) > that.editValue.maxMoney
