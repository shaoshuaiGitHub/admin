  ...
  <div style="margin: -24px -24px 0px;">
      ...
      <div class="ant-pro-grid-content">
        <div class="ant-page-header">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>还款诊断得分配置</a-breadcrumb-item>
          </a-breadcrumb>
          <div class="ant-page-header-heading">
            <span class="ant-page-header-heading-title">{{name+'_'+'还款诊断得分配置'}}</span>
          </div>
        </div>
      </div>
    </div>
    <div class="wrap-children-content">
      ...
      >
        ...
        >
          <template slot="allx" slot-scope="text, record">
          ...
          <template slot="ally" slot-scope="text, record">
            <template v-if="record.isDefault === 1">
              <a-icon
                type="check-circle"
                theme="twoTone"
            ...
            <template v-else>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[1].aid)"
              />
              <span style="margin-left:10px">%</span>
            </template>
          </template>
          <template slot="score" slot-scope="text, record">
            <div>
              <span style="margin:0 5px">{{"还款诊断得分是"}}</span>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[2].aid)"
              />
          ...
          <template slot="result" slot-scope="text,record">
            <a-select
              :defaultValue="text"
              style="width: 120px"
              @change="value=>handleChange2(value,record.cid,record.attributes[3].aid)"
            >
              <a-select-option
          ...
          <template slot="proposal" slot-scope="text, record">
            <div>
              <a-input
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[4].aid)"
              />
            </div>
          </template>
...
const columns = [
  {
    title: "近12个月内全额还款最小比例X%",
    width: "18%",
    dataIndex: "attributes[0].attrValue",
    scopedSlots: { customRender: "allx" }
  },
  {
    title: "近12个月内全额还款最大比例Y%",
    width: "18%",
    dataIndex: "attributes[1].attrValue",
    scopedSlots: { customRender: "ally" }
  },
  {
    title: "还款诊断得分（A）",
    width: "18%",
  ...
  },
  {
    title: "管理师建议",
    width: "20%",
    dataIndex: "attributes[4].attrValue",
    scopedSlots: { customRender: "proposal" }
  },
  {
    title: "操作",
    dataIndex: "operation",
    ...
    newPlan(status) {
      ...
      const newConfig = {
        attributes: [
          ...
          {
            attrKey: "CONSUM_QUOTAPRO_PROPOSAL",
            attrValue: "请编辑"
          }
        ],
        bankId: this.$route.params.id,
