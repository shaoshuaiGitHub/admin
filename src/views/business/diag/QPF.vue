      ...
      <div class="ant-pro-grid-content">
        <div class="ant-page-header">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>{{'预测提额率'}}</a-breadcrumb-item>
          </a-breadcrumb>
          <div class="ant-page-header-heading">
            <span class="ant-page-header-heading-title">{{name+'_'+'预测提额率'}}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      >
        ...
        >
          <template slot="attributes" slot-scope="text, record">
            <div>
              <template v-if="record.isDefault === 1">
                <a-icon type="check-circle" theme="twoTone" twoToneColor="#52c41a" :style="{ fontSize: '17px',marginLeft:'10px' }"/>
                <span style="margin-left:5px">{{'默认'}}</span>
              </template>
              <template v-else>
                <span style="margin:0 5px">{{"逾期"}}</span>
                <a-input
                  style="width:50px"
                  :defaultValue="text"
          ...
          <template slot="attributest" slot-scope="text, record">
            <div>
              <span style="margin:0 5px">{{"预测提额率"}}</span>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[1].aid,'attributest')"
              />
  ...
  methods: {
    ...
    newPlan(status) {
      ...
      const newConfig = {
        attributes: [
          {
            attrKey: "OVERDUE_TIMES",
            attrValue: "0"
          },
          {
            attrKey: "PRO_FORECAST",
            attrValue: "30"
          }
        ],
        bankId: this.$route.params.id,
      ...
      if (status == "default") {
         newConfig.attributes.splice(0, 1);
