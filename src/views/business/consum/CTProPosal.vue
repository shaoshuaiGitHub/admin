  ...
  <div style="margin: -24px -24px 0px;">
      ...
      <div class="ant-pro-grid-content">
        <div class="ant-page-header">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>消费时间建议</a-breadcrumb-item>
          </a-breadcrumb>
          <div class="ant-page-header-heading">
            <span class="ant-page-header-heading-title">{{name+'_'+'消费时间建议'}}</span>
          </div>
        </div>
      </div>
    </div>
    <div class="wrap-children-content">
      ...
      >
        ...
        >
          <template slot="mix" slot-scope="text, record">
            ...
            <template v-else>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[0].aid,'mix')"
              />
              <span style="margin-left:10px">笔</span>
            </template>
          </template>
          <template slot="max" slot-scope="text, record">
            ...
            <template v-else>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[1].aid,'max')"
              />
              <span style="margin-left:10px">笔</span>
            </template>
          </template>
          <template slot="proposal" slot-scope="text, record">
            <div>
              <a-input
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[2].aid,'proposal')"
              />
            </div>
          </template>
          <template slot="operation" slot-scope="text, record">
            <div class="editable-row-operations">
              <span>
                <a @click="() => deletes(record.cid)">删除</a>
              </span>
            </div>
          </template>
        </a-table>
...
const columns = [
  {
    title: "近12个月正常消费最少（X）笔",
    width: "25%",
    dataIndex: "attributes[0].attrValue",
    scopedSlots: { customRender: "mix" }
  },
  {
    title: "近12个月正常消费最多（Y）笔",
    width: "25%",
...
export default {
  ...
  methods: {
    ...
    //       that.tableLoading = false;
    //     }, 200);
    //     const defaultConfig = that.data.filter(item => 1 === item.isDefault)[0];
    //     if (defaultConfig) {
    //       that.hasDefaultL = true;
    //       defaultConfig.attributes.unshift("", "");
    //     } else {
    //       that.hasDefaultL = false;
    //     }
    //   });
    // },
    newPlan(status) {
      ...
      const newConfig = {
        attributes: [
          {
            attrKey: "NORMAL_TIME_MIX",
            attrValue: "0"
          },
          {
            attrKey: "NORMAL_TIME_MAX",
            attrValue: "0"
          },
          {
            attrKey: "NORMAL_CONSUM_TIME_PROPOSAL",
            attrValue: "请编辑"
          },
        ],
        bankId: this.$route.params.id,
