  ...
  <div style="margin: -24px -24px 0px;">
      ...
      <div class="ant-pro-grid-content">
        <div class="ant-page-header">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>消费诊断得分配置</a-breadcrumb-item>
          </a-breadcrumb>
          <div class="ant-page-header-heading">
            <span class="ant-page-header-heading-title">{{name+'_'+'消费诊断得分配置'}}</span>
          </div>
        </div>
      </div>
    </div>
    <div class="wrap-children-content">
      ...
      >
        ...
        >
          <template slot="posx" slot-scope="text, record">
            ...
            <template v-else>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[0].aid,'posx')"
              />
              <span style="margin-left:10px">%</span>
            </template>
          </template>
          <template slot="posy" slot-scope="text, record">
            <template v-if="record.isDefault === 1">
              <a-icon
                type="check-circle"
                theme="twoTone"
            ...
            <template v-else>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[1].aid,'posy')"
              />
              <span style="margin-left:10px">%</span>
            </template>
          </template>
          <template slot="score" slot-scope="text, record">
            <div>
              <span style="margin:0 5px">{{"消费诊断得分是"}}</span>
              <a-input
                style="width:50px"
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[2].aid,'score')"
              />
          ...
          <template slot="description" slot-scope="text, record">
            <div>
              <a-input
                :defaultValue="text"
                @change="e => handleChange2(e.target.value, record.cid, record.attributes[3].aid,'description')"
              />
            </div>
          </template>
