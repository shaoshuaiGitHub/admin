...
class WithdrawMoneyManagerPage extends StatefulWidget {
  final int type; //1未设置密码 2已设置
  WithdrawMoneyManagerPage({
    Key key,
    @required this.type,
  }) : super(key: key);
  @override
...
class _WithdrawMoneyPage extends State<WithdrawMoneyManagerPage> {
      ...
      child: Scaffold(
        ...
        body: BlocProvider<WithdrawBloc>(
          ...
          child: Container(
            ...
            child: Stack(
              children: <Widget>[
                MyWithdrawMoneyMiddle(type: this.type),
                Positioned(
                  child: MyMenusWidget(
                    offStage: offStage,
                  ),
...
class MyWithdrawMoneyMiddle extends StatefulWidget {
  int type; //1 未设置 2 已设置
  MyWithdrawMoneyMiddle({
    Key key,
    @required this.type,
  }) : super(key: key);
  @override
  State<StatefulWidget> createState() => _MyWithdrawMoneyMiddle(
        type: this.type,
      );
}

class _MyWithdrawMoneyMiddle extends State<MyWithdrawMoneyMiddle> {
  //页面
  ...
  void changeMoney(val) {
    moneyInputCrl.text = val.toDouble().toString();
  }
...
class MyBankList extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _MyBankList(
        selectedId: this.cardInstId,
      );
}

class _MyBankList extends State<MyBankList> {
  int selectedId;
  _MyBankList({
    Key key,
...
//导航
class MyMenusWidget extends StatelessWidget {
  final bool offStage;
  const MyMenusWidget({Key key, this.offStage}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Offstage(
      offstage: !offStage,
