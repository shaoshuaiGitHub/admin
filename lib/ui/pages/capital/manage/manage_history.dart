...
class AddManageHistory extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _AddManageHistory();
}

class _AddManageHistory extends State<AddManageHistory>
  ...
  Widget build(BuildContext context) {
    ...
    if (isOpen) {
      Future.delayed(new Duration(milliseconds: 100)).then((_) {
        planBloc.getHistoryPlan(dateTime, loadType, isBack);
      });
    ...
    return Stack(
      children: <Widget>[
        EasyRefresh(
          ...
          onRefresh: () async {
            planBloc.selPanel = false;
            planBloc.getHistoryPlan(dateTime, 2, false);
          },
          loadMore: () async {
            planBloc.selPanel = false;
            planBloc.getHistoryPlan(dateTime, 1, false);
          },
        ),
        Positioned(
          child: Container(
                          ...
                          if (val != null) {
                            if (val.length == 1) {
                              ...
                              isSame = beDate
                                  ...
                                  "0:00:00";
                              if (isSame) {
                                startTime = beDate.toString().substring(0, 11) +
                                    '00:00:00';
...
class _MyList extends State<MyList> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      stream: planBloc.outHistoryPlan,
    ...
    for (int i = 0; i < planListData.length; i++) {
      ...
      planList.add(
        ExpansionPanelCustom(
          ...
          body: Container(
            child: MyDetail(planItem: dataItem),
          ),
